...
def drawing(string):
    if string == "師生數量":
        ...
        response = requests.get("https://i.imgur.com/BVYyjt3.jpg")
        chinese = Image.open(BytesIO(response.content))
        width, height = chinese.size
        area = [145, 25, 145 + width, 25 + height]
        im.paste(chinese, area)

        response = requests.get("https://i.imgur.com/tVudGrj.jpg")
        chinese = Image.open(BytesIO(response.content))
        width, height = chinese.size
        area = [415, 25, 415 + width, 25 + height]
        im.paste(chinese, area)

        response = requests.get("https://i.imgur.com/satsmEn.jpg")
        chinese = Image.open(BytesIO(response.content))
        width, height = chinese.size
        area = [145, 225, 145 + width, 225 + height]
        im.paste(chinese, area)
